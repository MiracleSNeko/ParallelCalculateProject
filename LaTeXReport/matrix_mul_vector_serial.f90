`!******************************************************************************`
`!`
`!  matrix\_mul\_vector\_serial.f90`
`!  串行矩阵向量乘法`
`!`
`!******************************************************************************`
...
program serial_Mat_mul_Vec
    ...
    `! 读取矩阵`
    open(10, file = 'matrix', access = 'direct', form = 'unformatted', recl = 4*N*N)
    ...
    `! 读取向量`
    open(20, file = 'vector', access = 'direct', form = 'unformatted', recl = 4*N)
    ...
    `! 输出结果到向量文件`
    open(30, file = 'answer', access = 'direct', form = 'unformatted', recl = 4)
    do i = 1, N
        write(30, rec = i) answer(i, 1)
    end do
    
    call cpu_time(endwtime)
    open(40, file = 'walltime', access = 'direct', form = 'unformatted', recl = 4)
    write(40, rec = 1) (endwtime - startwtime) * 1000
    close(40)
